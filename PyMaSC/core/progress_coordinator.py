...
class ProgressCoordinator:
    ...
    def handle_multiprocess_report(self, chrom: str, pos: int) -> None:
        """Handle progress report from worker process.

        Args:
            chrom: Chromosome name
            progress_str: Actual progress text to show
        """
        if self._reporter is None:
            raise ValueError('Progress update requested before reporter initialization.')
        self._reporter.update(chrom, pos)
