...
def _parse_args() -> argparse.Namespace:
    ...
    if not Path(args.nreads).exists():
        parser.error("# of reads table file path does not exist: '{}'".format(args.nreads))
    elif all((args.cc and not Path(args.cc).exists(),
              args.masc and not Path(args.masc).exists())):
        ...
        if not Path(args.masc).exists():
            ...
            if args.mappability_stats and not args.mappability_stats.endswith(".json"):
                setattr(args, "mappability_stats",
                        str(Path(args.mappability_stats).with_suffix("")) + "_mappability.json")
            if not args.mappability_stats or not Path(args.mappability_stats).exists():
...
def main() -> None:
    ...
    for outputfunc, suffix in zip((output_stats, output_cc, output_mscc),
        ...
        if suffix in checked_suffixes:
            outputfunc(str(Path(args.outdir) / args.name), ccr)

    #
    plot_figures(str(Path(args.outdir) / (args.name + PLOTFILE_SUFFIX)), ccr)
...
def _prepare_outputs(args: argparse.Namespace) -> List[str]:
    ...
    output_base = str(Path(args.outdir) / args.name)
