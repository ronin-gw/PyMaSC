...
class MappabilityService(ABC):
    ...
    @abstractmethod
    def calculate_mappability_stats(
        self, chromosome: str, length: int
    ) -> MappabilityStats:
    ...
    @abstractmethod
    def calculate_genome_wide_stats(
    ...
    @abstractmethod
    def is_position_mappable(
        self, chromosome: str, position: int, threshold: float = 0.5
    ) -> bool:
...
def create_mappability_service(
    path: str, config: Optional[MappabilityConfig] = None
) -> MappabilityService:
