...
def _read_single_chromosome_worker(bam_path: str,
    ...
    This is a top-level function to avoid pickling issues with instance methods.
    """
...
class ParallelWorkflowService(StandardWorkflowService):
    ...
    def _read_chromosome_data(self,
        ...
        read_args = [
            (request.bam_path, chrom, request.calculation_config.mapq_criteria)
        ...
        with Pool(processes=self.n_workers) as pool:
            ...
            results = pool.starmap(_read_single_chromosome_worker, read_args)
            ...
            for i, data in enumerate(results):
                if data is not None:
                    ...
                    self._notify_progress(f"read_chromosome_{chromosomes[i]}", progress, 100)
                else:
                    # If reading failed, raise exception
                    raise RuntimeError(f"Failed to read chromosome {chromosomes[i]}")
    ...
    # The _read_single_chromosome method is now replaced by the top-level
    # _read_single_chromosome_worker function to avoid pickling issues
