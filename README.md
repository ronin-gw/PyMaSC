...
[![DOI:10.1186/s12859-020-03729-6](https://zenodo.org/badge/DOI/10.1186/s12859-020-03729-6.svg)](https://doi.org/10.1186/s12859-020-03729-6)
...
Introduction
------------
To estimate a mean fragment length for single-ended sequencing data, the cross-correlation
between positive- and negative-strand reads is commonly used. One of the problems
with this approach is _phantom_ peaks, which is the occasionally observed peak
corresponding to the read length. In the ChIP-Seq guidelines by ENCODE consortia,
cross-correlation at fragment length and read length are used for quality control
metrics. Additionally, library length itself is one of the important parameters
for analyses. However, estimating correct fragment length is not an easy task
because of _phantom_ peak occurrence.

P. Ramachandran _et al._ proposed __MaSC__ (mappability-sensitive cross-correlation)
to remove the bias caused by nonuniformity of mappability throughout the genome.
This method provides a cross-correlation landscape without _phantom_ peak and
much accurate mean fragment length estimation.

__PyMaSC__ is a tool implemented by python and cython to visualize mappability-sensitive
cross-correlation and estimate ChIP-Seq quality metrics and mean fragment length
with MaSC algorithm. Additionally, PyMaSC provides VSN (virtual S/N) metrics,
the value which reflects ChIP-Seq S/N (signal-to-noise ratio) level without peak
calling analysis.
...
### Install using pip
1. `numpy` and `pysam` must be installed ___before___ installing PyMaSC
...
#### Main input file
SAM and BAM file formats are acceptable.
* Input alignment file must be sorted.  
 * Additionally, for parallel processing, the input file must be BAM format and indexed.  
...
* If the input file contains paired-end reads, the last (second) segment read will be discarded.
...
Note that the progress bar will be disabled automatically if stderr is not
connected to a terminal.

##### --color {TRUE,FALSE}
Switch coloring log output. (Default: auto; enable if stderr is connected to a terminal)

#### --version
Show PyMaSC version number and exit.
...
PyMaSC needs a representative value of read length to plot figures and to calc
...
Specify how to get a representative value of read length. (Default: median)

##### -l / --library-length
Specify the expected fragment length. (Default: None)  
...
cross-correlation.  
The input file must be BigWig format and each track's score should indicate mappability
...
If there is no statistics file for specified BigWig file, PyMaSC calculates the total
lengths of doubly mappable regions for each shift size automatically and save them
to reuse for the next calculation and faster computing.  
...
##### -q / --mapq [int]
Input reads which mapping quality less than the specified score will be discarded. (Default: 1)  
...
PyMaSC performs chi-square test between the number of reads mapped to positive-
and negative-strand.

##### -w / --smooth-window [int]
Before the mean fragment length estimation, PyMaSC applies a moving average filter
to mappability-sensitive cross-correlation. This option specifies the filter's window
size. (Default: 15)

##### --mask-size [int]
If the difference between a read length and the estimated library length is equal or
...
overwrites the output file base name.
...
#### Usage example
Calculate the total lengths of doubly mappable regions.  
`wgEncodeCrgMapabilityAlign36mer_mappability.json` will be written.

    $ pymasc -p 4 -r 50 -d 1000 -m wgEncodeCrgMapabilityAlign36mer.bigWig

#### Options
Almost the same as `pymasc` command.  
...
Note that you need to specify a tab-delimited file (*.chrom.sizes) or a SAM/BAM
format file to obtain chromosome lengths, and/or, specify a mappability stats
(JSON) file which was generated for a mappability BigWig file by PyMaSC to obtain
mappable region lengths.
...
#### BitArray
BitArray approach is based on allocated binary arrays with a length of references.
The computation time mostly depends on the size of the reference genome and the
maximum shift size. PyMaSC consumes about 250MB RAM per worker for typical chromosomal sequences (hg19, chr1).

#### Successive implementation
Successive implementation is based on a buffer with the length of maximum shift
size. In this mode, PyMaSC calculates overwrapped bases successively while reading
reads and mappable regions. Computation time mostly depends on the number of reads
and tracks in input files and BitArray implementation is faster in most cases.
Successive approach surpasses in processing small input data, quite low memory
consumption and robust computation time for shift size.
...
* Anzawa, H., Yamagata, H. & Kinoshita, K. "Theoretical characterisation of strand
  cross-correlation in ChIP-seq." BMC Bioinformatics 21, 417 (2020).
